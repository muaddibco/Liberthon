...
namespace Wist.Node.Core.Synchronization
{
    ...
    public class TransactionsRegistrySyncHandler : IBlocksHandler
    {
        ...
        private readonly Dictionary<ulong, ManualResetEventSlim> _roundSwitches = new Dictionary<ulong, ManualResetEventSlim>();
        ...
        private void ProcessBlocks(CancellationToken ct)
        {
            foreach (RegistryBlockBase registryBlock in _registryBlocks.GetConsumingEnumerable(ct))
            {
                try
                {
                    ...
                    VerifyRoundSwitchExist(registryBlock.BlockHeight);

                    _roundSwitches[registryBlock.BlockHeight].Set();
        ...
        private void VerifyRoundSwitchExist(ulong round)
        {
            if (!_roundSwitches.ContainsKey(round))
            {
                lock (_roundSwitches)
                {
                    if (!_roundSwitches.ContainsKey(round))
                    {
                        _roundSwitches.Add(round, new ManualResetEventSlim(false));
                    }
                }
            }
        }
