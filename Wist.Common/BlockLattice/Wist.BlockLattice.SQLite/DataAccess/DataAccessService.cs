            ...
            get
            {
                if(_instance == null)
                {
                    lock(_sync)
                    {
                        if(_instance == null)
                        {
                            _instance = new DataAccessService(ServiceLocator.Current.GetInstance<IConfigurationService>(), ServiceLocator.Current.GetInstance<IIdentityKeyProvidersRegistry>(), ServiceLocator.Current.GetInstance<IHashCalculationsRepository>(), ServiceLocator.Current.GetInstance<ILoggerService>());
